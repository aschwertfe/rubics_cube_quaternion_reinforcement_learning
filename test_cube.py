...
def test_steps(cube, turns):
    ...
    for turn in range(turns):
        action = np.random.randint(low=0,high=3,size=2)
        dir = np.random.randint(low=0,high=2,size=1) *2 -1
        action = np.append(action, dir )
        print("Take action: ",action)
        done, reward, state, _ = cube.step(action)
        print("Reward: ", reward)
        print("State: \n", state)
